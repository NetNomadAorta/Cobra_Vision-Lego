...
elif RESNET_152_Toggle:
    from torchvision.models.detection.backbone_utils import resnet_fpn_backbone
    # Faster RCNN
    backbone = resnet_fpn_backbone('resnet152',
                                   pretrained=True
                                   )
    model_1 = models.detection.faster_rcnn.FasterRCNN(backbone,
                                                      n_classes_1,
                                                      min_size=IMAGE_SIZE,
                                                      max_size=IMAGE_SIZE*2
                                                      )
    SAVE_NAME_OD = "./Models/Vehicle_PPL_PPE-RN152.model"
    save_name_replcement = SAVE_NAME_OD.replace("-RN152", "")
    DATASET_PATH = "./Training_Data/" + save_name_replcement.split("./Models/", 1)[1].split(".model", 1)[0] + "/"
else:
    # Faster RCNN
